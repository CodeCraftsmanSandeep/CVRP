...
{
    Parameters par;
    par.set_theta_in_degrees(25.0);  // 5, 12, 25
...
// Prims algo
void insert_mst_edges(std::vector <std::vector <Edge>>& G, const CVRP& cvrp)
{
    size_t N = cvrp.size;
    node_t depot = cvrp.depot;

    std::priority_queue<
        std::pair <node_t, Edge>,
        std::vector<std::pair <node_t, Edge>>,
        std::function<bool(const std::pair <node_t, Edge>&, const std::pair <node_t, Edge>&)>
    > min_heap([](const std::pair <node_t, Edge>& e1, const std::pair <node_t, Edge>& e2) {
        return e1.second.w > e2.second.w; // Min-heap based on weight
    });

    std::vector<bool> in_mst(N, false);
    in_mst[depot] = true; // Start from depot
    node_t u, v;
    u = depot;
    std::vector<weight_t> min_value(N, INT_MAX);
    min_value[u] = 0;
    for(v = 0; v < N; v++)
    {
        if(v == depot) continue; // Skip depot
        weight_t distance = cvrp.get_distance(u, v);
        if(min_value[v] > distance)
        {
            min_heap.push({u, Edge(v, distance)});
            min_value[v] = distance;
        }
    }

    while(!min_heap.empty())
    {
        auto [u, edge] = min_heap.top();
        min_heap.pop();
        v = edge.v;
        weight_t w = edge.w;
        ...
        G[u].push_back(edge);
        G[v].push_back(Edge(u, w)); // considering MST egde as undirected

        for(node_t vertex = 0; vertex < N; vertex++)
        {
            if(in_mst[vertex]) continue;
            w = cvrp.get_distance(vertex, v);
            if(w > min_value[vertex]) continue;
            if(w < min_value[vertex])
            {
                // Update the min_value and push to the heap
                min_value[vertex] = w;
                min_heap.push({v, Edge(vertex, w)});
            }
        }
    }
}
...
void run_our_method(const CVRP& cvrp, Parameters& par, const CommandLineArgs& command_line_args)
{
    ...
    {
        // Step 1) for each vertex giving nearest D out neighbours
        std::priority_queue<
            Edge,
            std::vector<Edge>,
            std::function<bool(const Edge&, const Edge&)>
        > pq([](const Edge& e1, const Edge& e2) {
            return e1.w < e2.w; // Max-heap based on weight
        });
        ...
        // Step 2) Insert into MST edges
        insert_mst_edges(G, cvrp);
    }

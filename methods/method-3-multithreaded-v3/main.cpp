...
void create_aux_graph(std::vector <std::vector<int>>& adj, std::vector <int>& depot_neighbours, const std::vector <node_t>& bucket, const CVRP& cvrp, const Parameters& par) {
    ...
    std::priority_queue<
        ...
        });  // TODO: we should develop custom priority queue to support decrease key operation in min heap
    ...
    const int depot = 0;
    int u_index = 0; // Start from the first node in the bucket (which is depot)

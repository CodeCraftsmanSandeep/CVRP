...
class VRP {
  ...
  std::vector<std::vector<Edge>> cal_graph_dist_mine();
...
VRP::cal_graph_dist_mine() {
    // select closest 30 nodes for each vertex
    std::vector<std::vector<Edge>> nG(size);
    

    for(int i = 0; i < size; i++)
    {
        std::priority_queue< std::pair<weight_t, int>> pq; // max heap

        for(int j = 0; j < size; j++)
        {
            if(j == i) continue;
            weight_t w = sqrt(((node[i].x - node[j].x) * (node[i].x - node[j].x)) + ((node[i].y - node[j].y) * (node[i].y - node[j].y)));
            w = (params.toRound ? round(w) : w);  //TO round or not to.
            if(pq.size() < MIN_NODES)
            {
                pq.push({w, j});
            }else if(pq.top().first > w)
            {
                pq.pop();
                pq.push({w, j});
            }
        ...
        while(!pq.empty())
        {
            nG[i].push_back(Edge(pq.top().second, pq.top().first));
            pq.pop();
        }
    }
    return nG;
...
int main(int argc, char *argv[]) {
  ...
  auto G  = vrp.cal_graph_dist();
  auto mstG =   vrp.cal_graph_dist_mine(); // graph such that top nodes are selected for each vertex as neighbours
  auto mstEdges = PrimsAlgo(vrp, G);

  for(int u = 0; u < vrp.getSize(); u++)
  {
    for(auto e: mstEdges[u])
    {
      mstG[u].push_back(e);
    }
  }
  ...
  for (int i = 0; i < 1; i++) {
    ...
    for (auto &list : mstG) {                                              //& indicates the exiting mst list will be modified and subsequent Shortcircuit computation
    ...
    ShortCircutTour(mstG, visited, 0, singleRoute);  //a DFS //Write singleRoute
    DEBUG std::cout << '\n';

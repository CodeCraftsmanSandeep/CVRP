...
// DFS Recursive.
void ShortCircutTour(const VRP &vrp, std::vector<std::vector<Edge>> &g, std::vector<bool> &visited, node_t u, std::vector<node_t> &out, std::vector <bool>& consider, weight_t& c, bool& reject) {
  visited[u] = true;
  DEBUG std::cout << u << ' ';
  ...
  for (auto e : g[u]) {
    node_t v = e.to;
    if (!visited[v]) {
      int d = vrp.node[v].demand;
      if(c - d >= 0)
      {
        consider[v] = false;
        c -= d;
        ShortCircutTour(vrp, g, visited, v, out, consider, c, reject);
      }else{
        reject = true;
      }
      if(reject) return;
...
std::vector<std::vector<Edge>>
getMST(const VRP &vrp, std::vector<std::vector<Edge>> &graph, std::vector<bool>& consider) {
  auto N = graph.size();
  const node_t INIT = -1;
  //! std::cout<< "N "<< N << '\n';
...
int main(int argc, char *argv[]) {
  ...
  // minRoute, minCost is the slution tuple
  weight_t minCost = INT_MAX * 1.0f;
  std::vector<std::vector<node_t>> minRoute;
  int n = vrp.getSize();
  vector <bool> consider(n, true);


  while(n > 1){
    auto mstG = getMST(vrp, cG, consider);
    ...
    auto c = vrp.getCapacity();
    bool reject = false;

    ShortCircutTour(vrp, mstG, visited, 0, singleRoute, consider, c, reject);  //a DFS
    if(singleRoute.size() == 1)
    {
      break;
    }
    minRoute.push_back(singleRoute);
    n -= (singleRoute.size() - 1);
    DEBUG std::cout << '\n';
  }
  minCost = calCost(vrp, minRoute).first;
